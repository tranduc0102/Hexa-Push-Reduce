...
namespace Kamgam.UVEditor
{
    ...
    /// 1) First it searches for nearby SkinnedMeshRenderers (two levels up and down in the hierarchy).<br />
    ...
    ///    hierarchy does not match the mesh bind pose order (it often does not).
    /// <br /><br />
    ...
    public class BoneDataResolver : MonoBehaviour
    {
        ...
        public bool Resolve()
        {
            if (BoneData != null && Renderer != null)
            {
                ...
                // We check for the parent of the parent because that's how the MeshSmoother may have set
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (_infoFoldout)
            {
                EditorGUILayout.HelpBox(new GUIContent(
                        ...
                        "1) First it searches for nearby SkinnedMeshRenderers (two levels up and down in the hierarchy).\n" +
                        "2) If no renderers are found it will try to find a sibling named like BoneData.RootName and use\n" +
                        "   that as root bone. NOTICE: This may lead to distortions if the order of the transforms in the\n" +
                        "   hierarchy does not match the mesh bind pose order (it often does not).\n" +
                        "\n\n" +
